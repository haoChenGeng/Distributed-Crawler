  ...
  render() {
    ...
    const more =
      this.props.needMore === 'true' ? (
        <a onClick={this.showTotal.bind(this)} className="showmore">
          更多
        </a>
      ) : null;
    return (
      <Card title={name} extra={more}>
        <Radio.Group value={size} onChange={this.handleDateChange.bind(this)} style={{ marginBottom: 10 }}>
        ...
        {this.props.needMore === 'true' ? (
          <IndexTable columns={this.props.columns} pagination={false} />
        ) : (
          <ChartViews option={this.props.option} />
        )}
        <Modal
          title="更多"
